...
{
JointTrajectoryController::JointTrajectoryController():controller_interface::ControllerInterface(), dof_(0), num_cmd_joints_(0) {
}
...
JointTrajectoryController::set_hold_position() {
  ...
  // Clear historical data when configuring
  clear_historical_data();

  return hold_position_msg_ptr_;
}

#pragma endregion on_configure()

#pragma region ROS topic/service/action handlers
...
rclcpp_action::CancelResponse JointTrajectoryController::goal_cancelled_callback(
  ...
  if (active_goal && active_goal->gh_ == goal_handle)
  {
    ...
    // Add historical data to result (using DUMMYX as placeholder)
    // TODO: Replace DUMMYX with actual field names when FollowJointTrajectory supports them
    // action_res->DUMMYX_desired_states = historical_desired_states_;
    // action_res->DUMMYX_current_states = historical_current_states_;
    // action_res->DUMMYX_state_errors = historical_state_errors_;
    // action_res->DUMMYX_timestamps = historical_timestamps_;
    ...
    // Clear historical data when canceling a goal
    clear_historical_data();
  }
...
void JointTrajectoryController::goal_accepted_callback(
  ...
  // Update new trajectory
  {
    ...
    // Clear historical data when accepting a new goal
    clear_historical_data();
  }
...
void JointTrajectoryController::preempt_active_goal() {
  ...
  // Clear historical data when preempting a goal
  clear_historical_data();
}
...
#pragma endregion ROS topic/service/action handlers
  ...
  if (has_active_trajectory())
  {
    ...
    if (valid_point)
    {
      ...
      if (active_goal)
      {
        ...
        store_historical_data(state_desired_, state_current_, state_error_, time);
        ...
        if (tolerance_violated_while_moving)
        {
          ...
          // Add historical data to result (using DUMMYX as placeholder)
          // TODO: Replace DUMMYX with actual field names when FollowJointTrajectory supports them
          // result->DUMMYX_desired_states = historical_desired_states_;
          // result->DUMMYX_current_states = historical_current_states_;
          // result->DUMMYX_state_errors = historical_state_errors_;
          // result->DUMMYX_timestamps = historical_timestamps_;
...
void JointTrajectoryController::store_historical_data(
...
{
  // Store historical data for action results
  historical_desired_states_.push_back(desired_state);
  historical_current_states_.push_back(current_state);
  historical_state_errors_.push_back(state_error);
  historical_timestamps_.push_back(timestamp);
}

void JointTrajectoryController::clear_historical_data()
{
  // Clear historical data when starting a new trajectory
  historical_desired_states_.clear();
  historical_current_states_.clear();
  historical_state_errors_.clear();
  historical_timestamps_.clear();
}
...
controller_interface::CallbackReturn JointTrajectoryController::on_deactivate(
  ...
  // Clear historical data when deactivating
  clear_historical_data();
...
#pragma region read_from_interfaces()
...
#pragma endregion read_from_interfaces()
