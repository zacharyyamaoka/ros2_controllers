  ...
  double current_velocity)
{
  ...
  {
    // const double elapsed_time = (time - last_movement_time_).seconds();
    // RCLCPP_INFO(get_node()->get_logger(),
    //   "Checking for stall:\n"
    //   " - current time: %.3f\n"
    //   " - last movement time: %.3f\n"
    //   " - elapsed: %.3f\n"
    //   " - stall timeout: %.3f",
    //   time.seconds(), last_movement_time_.seconds(), elapsed_time, params_.stall_timeout);
